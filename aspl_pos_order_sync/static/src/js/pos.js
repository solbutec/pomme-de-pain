odoo.define('aspl_pos_order_sync.pos', function (require) {
	"use strict";
	...
	var SaleNoteListScreenWidget = screens.ScreenWidget.extend({
        ...
        click_reprint: function(event){
        	...
        	//console.log("=== SELF.POS ====/");
        	var result = self.pos.db.get_sale_note_by_id(order_id);
        	var selectedOrder = self.pos.add_new_order_synch(result);
        	selectedOrder.set_delivery_infos({
	    	    customer_name : (result.customer_name || '').trim(),
                customer_phone : (result.customer_phone || '').trim(),
                customer_addr : (result.customer_addr || '').trim(),
	    	});
        	if (result && result.lines.length > 0) {
    ...
    models.Order = models.Order.extend({
	    ...
	    get_delivery_infos: function(){
	        return {
                ...
                customer_addr : self.customer_addr || false,
               }
	    	},
...
//	        }
		...
		_save_to_server: function (orders, options) {
		 ...
		 add_new_order_synch: function(result) {
            if (this.config.iface_floorplan) {
                if(result.table_id){
                    var table = this.tables_by_id[result.table_id[0]];
                    this.set_table(table);
                }
                if (this.table) {
                    return _super_posmodel.prototype.add_new_order.call(this);
                } else {
                    console.warn("WARNING: orders cannot be created when there is no active table in restaurant mode");
                    return undefined;
                }
            } else {
                return _super_posmodel.prototype.add_new_order.apply(this, arguments);
            }
        },
	});
	...
	screens.ActionpadWidget.include({
            ...
            if (self.pos.config.enable_reorder){
                this.$('.pay').unbind('click').click(function(){
                    ...
                    if(order.table && order.table.is_for_delivery){
                            ...
                            var customer_name_v='', customer_phone_v='', customer_addr_v='';
                            if(current_customer){
                                customer_name_v = current_customer.name;
                                customer_phone_v = current_customer.phone;//.mobile
                                customer_addr_v = current_customer.address;
                            }
            	        	self.gui.show_popup('DeliveryInfosPopupWidget', {
                                order:current_order,
                                customer:current_customer,
                                customer_name: customer_name_v,
                                customer_phone: ''+customer_phone_v,
                                customer_addr: (customer_addr_v || '').trim(),
                                pad_widget: self,
                            });
        ...
        pay_after_delivery_infos: function(){
            var self = this;
            var order = self.pos.get_order();
            var has_valid_product_lot = _.every(order.orderlines.models, function(line){
                return line.has_valid_product_lot();
	...
	var DeliveryInfosPopupWidget = PopupWidget.extend({
	    ...
	    show: function(options){
	    	...
	    	self.order_select = options.order;
	    	self.customer_selected = options.customer;
	    	self.customer_name = options.customer_name;
            self.customer_phone = options.customer_phone;
            self.customer_addr = options.customer_addr;
	...
	gui.define_popup({name:'DeliveryInfosPopupWidget', widget: DeliveryInfosPopupWidget});
	...
	var ReorderProductPopupWidget = PopupWidget.extend({
	    ...
	    click_confirm: function(){
	    	...
	    	if(selected_ids && selected_ids[0]){
		    	...
		    	var order = self.pos.add_new_order_synch(self.old_order);
                order.set_delivery_infos({
                    customer_name : (self.old_order.customer_name || '').trim(),
                    customer_phone : (self.old_order.customer_phone || '').trim(),
                    customer_addr : (self.old_order.customer_addr || '').trim(),
                });
	...
	chrome.Chrome.include({
		...
		change_chosen_currency: function(event){
            ...
            //TODO : WHEN THE CURRENCY CHANGED WE MUST TO UPDATES VALUES IN PAYMENT LINE (call payment_input from PaymentScreenWidget)
